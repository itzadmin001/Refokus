...
scrollYProgress.on("change", (latest) => {
  ...
  return (
    ...
    <div className=' relative max-w-screen-xl mx-auto text-white text-center font-semibold tracking-tight select-none'>
        ...
        <p className=' text-zinc-500 mt-10'>Web Design, Webflow Development, Creative Development</p>
        <div className=' absolute w-full h-full top-0 '>
          {
            images.map((elem , i ) => {
              return  (
                elem.isActive && (<img src={elem.url} alt=""  className='absolute w-60 rounded-lg -translate-x-[ 50%] -translate-y-[50%]' style={{ top: elem.top , left : elem.left}}/>)
              )
            })
          }
        </div>
